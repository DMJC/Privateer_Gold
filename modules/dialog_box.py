...
def parse_dialog_box(args):
	...
	while i<len(args):
		...
		if type=='list':
		...
		elif type=='button' or type=='buttonspr' or type=='comp' or type=='compspr' or type=='room' or type=='roomspr':
			if type[:4]=='comp':
				arg=str(args[i]).split('/')
				i+=1
			if type[:4]=='room':
				arg=int(args[i])
				i+=1
			...
			if type[:4]=='comp':
				...
				currentList.append(DialogBox.RoomButton(id,arg,name,sprite,width,height))
			else:
		...
		elif type=='endrow' or type=='endcol':
			...
			listStack.append(currentList)
			currentList=[]
			elementsToCreate.append(DialogBox.Column(currentList))
		elif type=='position':
			i+=2 # x, y
		else:
...
class DialogBox:
	...
	class Column(Row):
		...
		def width(self):
			return reduce(max,map(lambda x:x.width(),self.items))
		def height(self):
			totalhei=0.
			for el in self.items:
				totalhei+=el.height()
		...
		def create(self,owner,room,x,y,wid,hei):
			...
			if self.items:
				...
				for it in self.items:
					h = it.height()
					it.create(owner,room,x,y,wid,h)
					y -= h
					if it.gui_element:
						childs.append(it.gui_element)
				self.gui_element.children = childs
	...
	def create(self,roomid):
		if VS.isserver():
			return
		self.calculateWidth()
		x=self.totalwidth/-2. + self.xcent
		y=self.totalheight/2. - self.ycent
		room=GUI.GUIRootSingleton.getRoomById(roomid)
		if not room:
...
def fromValues(data):
	action = data[0]
	value_dict = dict(zip(data[1::2],data[2::2]))
	return action, value_dict
...
def alert(message, callback=None, width=1.0, buttonText='OK', x=0.,y=0.):
	...
	prepend=[]
	if x!=0 or y!=0:
		prepend=["position",x,y]
	dialog(prepend + ["width",width, "text",message] + button_row(width, buttonText), dbcallback)
