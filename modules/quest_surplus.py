...
def makeSurplusShortage():
    ...
    news=""
    if (adj<0):
        news = "SURPLUS IN %s:  There are reports of surpluses of %s and %s items in the %s system.  Wise Privateers have been purchasing the surpluses in hopes of future gains elsewhere." % (sys,cats[0],cats[1],sys)
    else:
        news = "DEMAND FOR %s AND %s in %s:  Citizens in the %s system have been frusterated by the expensive prices and low quantities of %s and %s goods.  Privateers are currently banding together to try to bring supply to meet the demand.  But while prices are still high, some big traders could certainly capitalize on this shortage." % (cats[0],cats[1],sys,sys,cats[0],cats[1])
